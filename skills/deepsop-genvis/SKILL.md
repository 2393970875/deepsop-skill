...
- `methodType=10/11` 参考图必须是可公开访问的 HTTP/HTTPS 图片 URL，单张大小不得超过 10MB，支持 JPG/JPEG、PNG、WebP、GIF，`prompt` 最长 16000 字。
- 参考图提交前必须校验 HTTP 状态、图片内容类型、实际文件大小和文件格式；URL 不可访问、响应不是图片或超过 10MB 时，必须停止提交，不得进入费用预估和创建任务。
