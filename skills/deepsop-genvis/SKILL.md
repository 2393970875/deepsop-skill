...
- 用户询问“用什么模型 / 适合哪个模型 / 某类视频或图片对应什么模型 / 数字人带货视频用什么模型”等模型咨询时，必须调用 `scripts/generate_image.py "<用户原话>" --recommend-model --json-output`，依据接口返回模型的 `sourceName/sourceValue/sourceType/sourceDescription` 回答；不得凭记忆、外部旧名称或本地示例回答。该推荐查询必须跨 `IMAGE_MODEL`、`VIDEO_MODEL`、`HUMAN_MODEL` 三类按描述打分，不能因为用户提到“数字人”就只筛 `HUMAN_MODEL`。
