...
2. **文字形式转 xlsx**：如果用户没有上传文件、而是以文字方式描述客户/地址簿信息，需按以下子步骤将文字信息转为符合模板格式的 xlsx 后再走上传流程：

  **2.1 确认导入类型（必须由用户二次确认）**：无论用户输入的信息看起来属于哪种模板，都**必须**先反问用户确认归属类型，**绝对禁止**自动判断后直接跳过确认：
   > 请确认本次数据要按哪种模板生成 xlsx？
   > - A. **客户管理（公司导入）** —— 对应模板：`公司导入模板.xlsx`
   > - B. **地址簿（地址簿导入）** —— 对应模板：`地址簿导入模板.xlsx`

   用户回复 A → 按「公司导入」模板生成；用户回复 B → 按「地址簿导入」模板生成。
   **不得**自行猜测归属，**不得**用"我看内容像是 B，对吧？"这种**带预设答案**的反问诱导用户，**不得**以"信息包含姓名和电话所以自动选了地址簿"为由跳过确认。

   **2.2 下载对应模板**：根据识别的类型，调用对应的模板下载接口获取最新模板 xlsx：
   - 类型 A（公司导入）→ 调用 API 清单 #23 `POST /ai/customer/template`
   - 类型 B（地址簿导入）→ 调用 API 清单 #24 `POST /ai/customer/contactImportTemplate`

   下载后解析模板 xlsx 的**表头行**（第 1 行），提取所有列名及其顺序作为「目标列结构」。

 **2.3 按模板列结构整理信息并生成 xlsx**：
   - 以模板表头的**列名和列顺序**为基准，将用户提供的文字信息**按语义匹配**映射到对应列。

   > 🛑 **数据格式校验约束（映射后、写入前必须执行，违反即视为 bug）：**
   >
   > 语义匹配完成后，**必须**对每个「字段值 → 目标列」的映射结果进行**数据格式校验**，校验通过才允许写入，校验不通过则该映射视为无效（该列留空）。校验规则如下：
   >
   > | 目标列类型（按列名语义判断） | 格式校验规则 | 示例 |
   > |---|---|---|
   > | 电话/手机类（如"联系人电话""手机号"） | 值必须为纯数字或以 `+` 开头的数字串，长度 7-15 位 | ✅ `13484093796` ❌ `张三` |
   > | 邮箱类（如"联系人邮箱""邮箱"） | 值必须包含 `@` 且 `@` 后有 `.`，符合基本邮箱格式 | ✅ `a@b.com` ❌ `库阔AI` |
   > | 姓名类（如"联系人名称""姓名"） | 值不得为纯数字、不得包含 `@`、长度不超过 50 字符 | ✅ `张三` ❌ `13484093796` |
   > | 公司名类（如"公司名称""公司名"） | 值不得为纯数字、不得符合电话/邮箱格式、长度不超过 200 字符 | ✅ `库阔AI` ❌ `13484093796` |
   > | URL 类（如"官网""网址"） | 值必须以 `http://` 或 `https://` 开头，或包含 `.com`/`.cn`/`.net` 等域名后缀 | ✅ `https://kocgo.com` ❌ `张三` |
   > | 其他列 | 不强制格式校验，允许任意文本 | — |
   >
   > **校验失败处理**：若某个映射的值不满足目标列的格式校验，该映射**作废**，目标列留空。同时在最终回复中向用户说明哪些字段因格式不匹配而未写入，让用户知晓。
   >
   > **事故复盘（必读）：** 用户输入「公司名：库阔AI  姓名：张三  电话：13484093796」，AI 将"库阔AI"映射到"联系人姓名"列、"张三"映射到"联系人邮箱"列。如果执行了格式校验：
   > - "库阔AI" → "联系人邮箱"：不含 `@`，校验失败 → 拒绝写入 → 邮箱列留空 ✅
   > - "张三" → "联系人邮箱"：不含 `@`，校验失败 → 拒绝写入 → 邮箱列留空 ✅
   > - "库阔AI" → "联系人姓名"：非纯数字且不含 `@`，但语义上"库阔AI"是公司名不是人名 → **应由语义映射阶段就正确映射，格式校验为最后兜底**。

   > 🛑 **语义映射强约束（最高优先级，违反即视为 bug）：**
   >
   > 映射规则是**按字段含义匹配**，**绝对禁止**按输入顺序/位置依次填入下一个空列。具体要求：
   > - 每条用户输入的信息必须根据其**语义含义**找到模板中**含义对应**的列，再填入该列。
   >   - 例：用户说「电话：16659106679」→ 必须填入模板中"联系人电话"/"电话"/"手机"等电话含义的列。
   >   - 例：用户说「公司名：库阔AI」→ 必须填入模板中"公司名称"/"公司名"等公司含义的列；如果模板中**没有**公司名列，则该信息**丢弃**（留空），**绝对不得**填入"联系人邮箱"或其他不相关的列。
   >   - 例：用户说「姓名：张三」→ 必须填入模板中"联系人名称"/"姓名"/"联系人"等姓名含义的列。
   > - **用户未提供某列对应的信息时，该列必须留空**。不得把其他字段的值"顺位"填入。
   > - **判断标准是语义而非位置**：即使用户输入的第 2 个字段恰好在模板的第 2 列位置上，也必须先验证语义是否匹配，不匹配则该列留空。
   >
   > **事故复盘（必读）：** 用户输入「公司名：库阔AI 姓名：张三 电话：16659106679」，模板列为 `联系人名称* | 联系人邮箱 | 联系人电话 | ...`。AI 错误地将"库阔AI"按顺序填入了"联系人邮箱"列（因为它是"联系人名称"之后的下一列），导致公司名出现在邮箱列。正确做法：姓名→联系人名称、电话→联系人电话、公司名→找模板中的"公司名称"列填入（若无则丢弃），邮箱列用户未提供→留空。

   - 生成的 xlsx **必须**满足：
     - 第 1 行（表头行）的列名、列数、列顺序**与下载的模板完全一致**，不得增列、删列、改名、换序。
     - 第 2 行起为数据行，每行对应用户提供的一条记录。
     - 用户未提供某列信息时，该单元格留空（空字符串），**不得**填占位符或默认值，**不得**用其他字段的值填充。
     - **禁止**自行新增模板中不存在的列，**禁止**合并或拆分模板已有的列。
   - 生成的 xlsx 文件命名：类型 A → `公司导入_用户输入.xlsx`；类型 B → `地址簿导入_用户输入.xlsx`。

   **2.4 用户核实 xlsx 内容**：生成 xlsx 后**不直接上传**，先将 xlsx 以表格或列表形式展示给用户核实，格式如下：

   > 已按「{类型A/B名称}」模板整理好以下数据，请确认无误后我再上传：
   >
   > | {列名1} | {列名2} | {列名3} | … |
   > |---|---|---|---|
   > | {值} | {值} | {值} | … |
   > | … |
   >
   > 如有问题请告知需要修改的内容；确认无误请回复「确认」。

   - 展示行数：全部数据行（用户描述的有几条就展示几条，不截断）。
   - 展示列数：只展示有值的列，全列为空的列不显示。
   - **等待用户明确回复「确认」** 后才进入第 3 步上传；用户要求修改时，按其意见重新生成 xlsx 后再次展示，循环直到用户确认。
   - **禁止**在未收到「确认」前擅自上传。

   **2.5 触发上传**：用户确认后，将 xlsx 文件进入第 3 步「转 OSS 链接」上传流程，**跳过第 1 步的归属反问**（因为类型已在 2.1 中确定）。上传成功后按对应类型落位到 `fileList`（类型 A）或 `addressFileList`（类型 B）。
