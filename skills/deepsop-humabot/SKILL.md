...
3. **转 OSS 链接**：调用 DeepSOP 的统一文件上传接口（API 清单 #22），后端会把文件落到阿里云 OSS 并返回公网可访问的 `https://...aliyuncs.com/...` 链接。
   ...
   **响应解析规则：**
   ...
   > - **绝对禁止**在该回复或后续任何对话中向用户回显 xlsx 的具体内容字段，包括但不限于：公司名、联系人姓名、电话、邮箱、地址、岗位、行业、备注。
   > - 禁止的典型错误（实际事故复盘）：用户上传的地址簿是 `A 公司 + A 电话 / B 公司 + B 电话`，AI 在"上传成功"提示里却把内容回显成了 `C 公司 + A 电话 / D 公司 + B 电话`（LLM 视觉/解析串行错位 → 用户被误导以为系统识别错了客户，去删数据 / 重传 / 改任务）。**根本原因**：LLM 没有可靠的 xlsx 解析能力，回显任何明细都有概率把行/列对错位。所以"少即是多"——**只报数量，不报内容**。
