 ...
 **2.3 按模板列结构整理信息并生成 xlsx**：
   ...
   **2.4 触发上传**：将生成的 xlsx 文件直接进入第 3 步「转 OSS 链接」上传流程，**跳过第 1 步的归属反问**（因为类型已在 2.1 中确定）。上传成功后按对应类型落位到 `fileList`（类型 A）或 `addressFileList`（类型 B）。
