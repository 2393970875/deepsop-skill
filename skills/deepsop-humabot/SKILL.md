...
2. **文字形式转 xlsx**：如果用户没有上传文件、而是以文字方式描述客户/地址簿信息，需按以下子步骤将文字信息转为符合模板格式的 xlsx 后再走上传流程：
   ...
   **2.1 识别导入类型**：根据用户输入的信息内容**自动判断**属于哪种模板：
   - 若信息以**公司维度**为主（含公司名称、行业、国家、官网、规模、营收等字段）→ 识别为「公司导入」（类型 A）
   - 若信息以**联系人维度**为主（含姓名、电话、邮箱、地址等个人联系方式字段）→ 识别为「地址簿导入」（类型 B）
   - 若无法明确判断，**必须**反问用户确认归属类型（反问话术同第 1 步），**不得**自行猜测。
