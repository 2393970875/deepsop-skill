#!/usr/bin/env python3
"""
submit_task.py（TikTokFlow / Toby 专用版）
...
调用方式（SKILL.md Step 3 的"提交"环节固定使用此模板）:
...
# ⚠️ 强约束：本脚本提交任务的 URL 必须与 SKILL.md「API 路径权威清单」一致；
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="DeepSOP agentSubmitTask 一站式提交（TikTokFlow / Toby 专用，UTF-8 安全 + pre-flight）",
    )
    parser.add_argument(
        "--file",
