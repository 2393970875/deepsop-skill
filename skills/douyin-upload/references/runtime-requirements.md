...
```bash
uv run --project <SAU_HOME> python sau_cli.py douyin <subcmd> [args...]
```
...
```powershell
# Windows PowerShell
uv run --project "$env:USERPROFILE\.openclaw\social-auto-upload" python sau_cli.py douyin --help
```

```bash
# macOS / Linux
uv run --project "$HOME/.openclaw/social-auto-upload" python sau_cli.py douyin --help
```

> 不要用 `sau douyin ...` 形式 —— 上游 `pyproject.toml` 虽然声明了 `[project.scripts] sau = "sau_cli:main"`，但 `conf.py` 是用户运行时复制的、不在 wheel 里，`uv tool install` 装出来的 `sau` 命令一定会 ImportError。**必须从源码目录用 uv run 调用**。
...
`python sau_cli.py douyin login --account <name>` **必须由用户自己在本机真实终端执行**：
